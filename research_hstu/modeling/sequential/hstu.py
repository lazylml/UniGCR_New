...
from .output_postprocessors import (
    OutputPostprocessorModule,
)
from .utils import (
    get_current_embeddings,
)
from research_hstu.modeling.similarity_module import (
...
# ================================
# Safe fallback for fbgemm ops
# ================================

def safe_complete_cumsum(lengths: torch.Tensor) -> torch.Tensor:
    """
    Replacement for torch.ops.fbgemm.asynchronous_complete_cumsum
    lengths: [B]
    return: [B+1] offsets
    """
    return torch.cat(
...
def safe_jagged_to_padded_dense(
    *,
    values: torch.Tensor,
    offsets,
    max_lengths=None,
    ...
    Compatible replacement for:
      1) jagged_to_padded_dense(values, offsets, max_length)
      2) jagged_to_padded_dense(values=..., offsets=[...], max_lengths=[...])

    Only supports SINGLE jagged dimension (which is exactly HSTU's case).
    """

    # ---- normalize arguments ----
    if max_lengths is not None:
        # fbgemm-style API
        assert isinstance(offsets, (list, tuple)) and len(offsets) == 1
        assert isinstance(max_lengths, (list, tuple)) and len(max_lengths) == 1
        offsets = offsets[0]
    ...
    out = values.new_full((B, max_length, D), padding_value)
...
def safe_dense_to_jagged(
    dense: torch.Tensor,
    offsets,
):
    """
    dense: [B, N, D]
    offsets: [x_offsets]  (length B+1)
    ...
    for i in range(B):
        ...
        length = end - start
        if length > 0:
            # 适配 Pre-padding: 从末尾倒数 length 个元素
            max_L = dense.size(1)
            values.append(dense[i, :length])

    if len(values) == 0:
        values = dense.new_empty((0, dense.size(-1)))
    else:
...
def _hstu_attention_maybe_from_cache(
    ...
    if delta_x_offsets is not None:
        ...
        # )
        padded_q = safe_jagged_to_padded_dense(
            values=q, offsets=[x_offsets], max_lengths=[n], padding_value=0.0
        )
        padded_k = safe_jagged_to_padded_dense(
            values=k, offsets=[x_offsets], max_lengths=[n], padding_value=0.0
        )
    ...
    #     [x_offsets],
    # )[0]
    attn_output = safe_dense_to_jagged(
        torch.einsum(
            "bhnm,bmhd->bnhd",
            qk_attn,
            safe_jagged_to_padded_dense(values=v, offsets=[x_offsets], max_lengths=[n],).reshape(
                B, n, num_heads, linear_dim
            ),
        ).reshape(B, n, num_heads * linear_dim),
        [x_offsets],
    ...
    def forward(  # pyre-ignore [3]
        ...
        if self._normalization == "rel_bias" or self._normalization == "hstu_rel_bias":
            ...
            attn_output = safe_dense_to_jagged(
                torch.bmm(
                    qk_attn,
                    safe_jagged_to_padded_dense(values=v, offsets=[x_offsets], max_lengths=[n]),
                ),
                [x_offsets],
            )[0]
        else:
...
class HSTUJagged(torch.nn.Module):
    ...
    def forward(
        ...
        if len(x.size()) == 3:
            ...
            x = safe_dense_to_jagged(x, [x_offsets])[0]
...
class HSTU(SequentialEncoderWithLearnedSimilarityModule):
    ...
    def __init__(
        ...
        self._embedding_dim: int = embedding_dim
        self._item_embedding_dim: int = embedding_module.embedding_dim
    ...
    def generate_user_embeddings(
        ...
        user_embeddings, cached_states = self._hstu(
            ...
            x_offsets=safe_complete_cumsum(past_lengths),
            all_timestamps=(
